...
const UserListTable = ({ tableData }: any) => {
  // States
  ...
  const handleEditUser = (userData: UsersType) => {
  ...
  const columns = useMemo<ColumnDef<UsersTypeWithAction, any>[]>(
    () => [
      ...
      columnHelper.accessor('isKycCompleted', {
        header: 'KYC',
        cell: ({ row }) => (
          <Chip
            label={row.original.isKycCompleted ? 'Completed' : 'Not Completed'}
            color={row.original.isKycCompleted ? 'success' : 'warning'}
            size='small'
            variant='tonal'
          />
        )
