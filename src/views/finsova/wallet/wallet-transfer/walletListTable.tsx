...
import AddUserDrawer from './AddwalletDrawer'
...
type UsersTypeWithAction = WalletType & {
  action?: string
}
  ...
  const columns = useMemo<ColumnDef<UsersTypeWithAction, any>[]>(
    () => [
      ...
      columnHelper.accessor('mobile', {
        header: 'Mobile',
        cell: ({ row }) => {
          const role = row.original.mobile || 'unknown'
          const roleData = userRoleObj[role] ?? { icon: 'tabler-user-question', color: 'secondary' }
      ...
      columnHelper.accessor('company', {
        header: 'Company',
        cell: ({ row }) =>
          <Typography>{row.original.company}</Typography>
      }),
      columnHelper.accessor('creditAmount', {
        header: 'Credit Amount',
        cell: ({ row }) => (
          <Typography>{row.original.creditAmount}</Typography>
        )
      }),
      columnHelper.accessor('debitAmount', {
        header: 'Debit Amount',
        cell: ({ row }) => (
          <Typography>{row.original.debitAmount}</Typography>
        )
      }),
