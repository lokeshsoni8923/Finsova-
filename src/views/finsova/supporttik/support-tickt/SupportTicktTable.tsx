...
import AddUserDrawer from './AddSupportTicktModal'
...
type UsersTypeWithAction = SupportTicktType & {
  action?: string
}
...
const WalletlistTable = ({ tableData }:any) => {
  // States
  ...
  const columns = useMemo<ColumnDef<UsersTypeWithAction, any>[]>(
    () => [
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-4'>
            <div className='flex flex-col'>
              <Typography variant='body2'>{row.original.lastUpdateDate}</Typography>
            </div>
          </div>
        )
      }),
      columnHelper.accessor('ticketNumber', {
        ...
        cell: ({ row }) => {
          const role = row.original.ticketNumber || 'unknown'
          return (
      ...
      columnHelper.accessor('service', {
        header: 'Service',
        cell: ({ row }) =>
          <Typography>{row.original.service}</Typography>
      }),
      columnHelper.accessor('txnDate', {
        header: 'Transaction Date',
        cell: ({ row }) => (
          <Typography>{row.original.txnDate}</Typography>
        )
      }),
      columnHelper.accessor('referenceNumber', {
        header: 'Reference Number',
        cell: ({ row }) => (
          <Typography>{row.original.referenceNumber}</Typography>
        )
      }),
      ...
      columnHelper.accessor('dateCreated', {
        header: 'Date Created',
        cell: ({ row }) => (
          <Typography>{row.original.dateCreated || 'N/A'}</Typography>
        )
      }),
